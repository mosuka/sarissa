/**
 * OPFS (Origin Private File System) helper utilities for Laurus WASM.
 *
 * Provides functions to download, store, load, and manage Lindera
 * dictionaries using the browser's Origin Private File System for
 * persistent caching. Used together with `JapaneseAnalyzer.fromBytes()`
 * to enable Japanese morphological analysis in browser WASM, where
 ...
 *
 * Parses the zip central directory and decompresses entries with the
 * Web Streams API, avoiding any external zip library dependency.
 *
 ...
 *
 * The returned object's keys match the camelCase argument names
 * expected by `JapaneseAnalyzer.fromBytes()`.
 *
 * @param {string} name - The dictionary name (e.g., "ipadic").
 * @returns {Promise<object>} Object containing the dictionary file data.
