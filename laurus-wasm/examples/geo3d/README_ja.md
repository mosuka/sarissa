...
- 軌道要素は
  ...
  CelesTrak は **IP・グループ毎に、次のデータ更新（約 2 時間毎）まで
  HTTP 403** を返すため、デモはダウンロードしたグループを Cache API
  で永続化します。更新枠内のリロードは全てキャッシュから供給され、
  キャッシュがある状態での 403 は「手元のコピーが最新」の意味として
  そのまま利用します。キャッシュなしで 403 になった場合（同一 IP の
  別ブラウザ等）は、別グループへの切り替えか次の更新後の再試行を
  案内します。
