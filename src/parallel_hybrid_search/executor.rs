...
use super::types::{
    HybridSearchTask, HybridSearchTaskResult, IndexSearchStats,
    ParallelHybridSearchResults, SearchTimeBreakdown,
};
...
impl ParallelHybridSearchExecutor {
    ...
    pub async fn execute_parallel_search(
        ...
        // Create search tasks
        let tasks = self.create_search_tasks(
            query_text,
            keyword_query,
            query_vector,
            indices,
        ...
        let merged_results = self.merger.merge(
            keyword_results,
            vector_results,
            document_store,
        );
    ...
    fn execute_single_task(
        ...
        *keyword_time.lock().unwrap() += kw_time;
        ...
        let vector_results = if let (Some(vector_reader), Some(query_vector)) = 
            ...
            match Self::execute_vector_search(
                query_vector,
                vector_reader,
                config,
            ) {
    ...
    fn execute_vector_search(
        ...
        Err(SarissaError::NotImplemented("Vector search not implemented in this example".to_string()))
    }
