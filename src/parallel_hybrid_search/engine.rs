...
impl ParallelHybridSearchEngine {
    ...
    pub async fn remove_index(&self, id: &str) -> Result<()> {
        let mut indices = self.indices.write().await;
        indices.remove(id).ok_or_else(|| {
            SarissaError::internal(format!("Index '{id}' not found"))
        })?;
        Ok(())
    }
    ...
    pub async fn search(
        ...
        let results = self.executor.execute_parallel_search(
            query_text,
            keyword_query,
            query_vector,
            indices,
...
mod tests {
    ...
    async fn test_add_remove_index() {
        ...
        assert!(engine.add_index("index1".to_string(), reader, None, 1.0).await.is_ok());
