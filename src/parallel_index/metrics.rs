...
impl ThroughputWindow {
    ...
    fn get_current_throughput(&self) -> (f64, f64) {
        ...
        if let (Some((oldest_time, _)), Some((newest_time, _))) = 
            (self.recent_operations.front(), self.recent_operations.back()) {
...
impl IndexingMetricsCollector {
    ...
    pub fn new() -> Self {
        Self {
            ...
            throughput_window: Arc::new(parking_lot::Mutex::new(
                ThroughputWindow::new(Duration::from_secs(60))
            )),
        }
    }
    ...
    pub fn record_operation(
        ...
        if success {
            ...
            self.total_documents_indexed.fetch_add(doc_count, Ordering::Relaxed);
        } else {
    ...
    pub fn record_commit(&self, commit_time: Duration) {
        ...
        self.total_commit_nanos.fetch_add(commit_time.as_nanos() as u64, Ordering::Relaxed);
    }
    ...
    pub fn snapshot(&self) -> IndexingMetrics {
        ...
        let (current_docs_per_sec, current_ops_per_sec) = throughput_window.get_current_throughput();
        ...
        IndexingMetrics {
            ...
            max_execution_time: Duration::from_nanos(self.max_execution_nanos.load(Ordering::Relaxed)),
            min_execution_time: min_duration,
            total_commits: self.total_commits.load(Ordering::Relaxed),
            total_commit_time: Duration::from_nanos(self.total_commit_nanos.load(Ordering::Relaxed)),
