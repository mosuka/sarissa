...
pub trait DocumentPartitioner: Send + Sync {
    ...
    fn validate(&self) -> Result<()> {
        if self.partition_count() == 0 {
            return Err(SarissaError::invalid_argument("Partition count cannot be zero"));
        }
        Ok(())
    }
}
...
impl DocumentPartitioner for HashPartitioner {
    fn partition(&self, doc: &Document) -> Result<usize> {
        let field_value = doc.get_field(&self.field_name)
            .ok_or_else(|| SarissaError::field(format!(
...
impl RangePartitioner {
    ...
    pub fn new(field_name: String, boundaries: Vec<i64>) -> Result<Self> {
        if boundaries.is_empty() {
            return Err(SarissaError::invalid_argument("Range boundaries cannot be empty"));
        }
    ...
    pub fn new_date_ranges(field_name: String, date_ranges: Vec<&str>) -> Result<Self> {
        ...
        for date_str in date_ranges {
            let timestamp = chrono::DateTime::parse_from_rfc3339(date_str)
                .map_err(|e| SarissaError::invalid_argument(format!(
...
impl DocumentPartitioner for RangePartitioner {
    fn partition(&self, doc: &Document) -> Result<usize> {
        ...
        let numeric_value = match field_value {
            ...
            crate::schema::FieldValue::Text(s) => {
                s.parse::<i64>()
                    .map_err(|_| SarissaError::field(format!(
                        "Cannot convert field value '{s}' to integer"
                    )))?
            }
            _ => return Err(SarissaError::field(format!(
...
impl ValuePartitioner {
    ...
    pub fn add_mapping(mut self, value: String, partition: usize) -> Result<Self> {
        if partition >= self.partition_count {
            return Err(SarissaError::invalid_argument(format!(
                ...
                partition, self.partition_count - 1
            )));
        }
    ...
    pub fn from_mapping(
        ...
        if mapping.is_empty() {
            return Err(SarissaError::invalid_argument("Value mapping cannot be empty"));
        }
...
mod tests {
    use super::*;
    use crate::schema::{FieldValue, Document};
