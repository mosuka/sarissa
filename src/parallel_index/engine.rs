...
use crate::parallel_index::{
    batch_processor::{BatchProcessor, BatchProcessingResult},
...
impl ParallelIndexEngine {
    ...
    pub fn index_documents(
        ...
        if writers.is_empty() {
            return Err(SarissaError::invalid_argument("No active writers available"));
        }
        ...
        // Aggregate results
        let documents_indexed: usize = results.iter()
            .map(|r| r.documents_indexed)
            .sum();
        
        let documents_failed: usize = results.iter()
            .map(|r| r.documents_failed)
            .sum();
    ...
    fn find_writer_for_partition(
        ...
        writer_map.get(&partition_id)
            .cloned()
            .or_else(|| {
                // Fallback: use any available writer if exact match not found
                writer_map.values().next().cloned()
            })
            .ok_or_else(|| SarissaError::not_found(format!(
                "No writer found for partition {partition_index}"
            )))
    }
...
mod tests {
    ...
    fn create_test_writer() -> Box<dyn IndexWriter> {
        ...
        schema.add_field("content", Box::new(TextField::new())).unwrap();
        let storage = Arc::new(MemoryStorage::new(StorageConfig::default()));
        Box::new(BasicIndexWriter::new(schema, storage, crate::index::writer::WriterConfig::default()).unwrap())
    }
    ...
    fn test_partition_management() {
        ...
        engine.add_partition("partition_0".to_string(), create_test_writer(), partition_config1).unwrap();
        
        let partition_config2 = PartitionConfig::new("partition_1".to_string());
        engine.add_partition("partition_1".to_string(), create_test_writer(), partition_config2).unwrap();
    ...
    fn test_commit_all() {
        ...
        engine.add_partition("partition_0".to_string(), create_test_writer(), partition_config).unwrap();
