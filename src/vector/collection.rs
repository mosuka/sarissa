...
impl VectorCollection {
    ...
    pub fn config(&self) -> &VectorCollectionConfig {
        self.config.as_ref()
    }
    ...
    pub fn stats(&self) -> Result<VectorCollectionStats> {
    ...
    pub fn search(&self, query: &VectorCollectionQuery) -> Result<VectorCollectionSearchResults> {
        ...
        if query.limit == 0 {
            return Ok(VectorCollectionSearchResults::default());
        }

        if query.overfetch < 1.0 {
            return Err(PlatypusError::invalid_argument(
                "VectorCollectionQuery overfetch must be >= 1.0",
            ));
        }
        ...
        let mut doc_hits: HashMap<u64, VectorCollectionHit> = HashMap::new();
        ...
        let mut hits: Vec<VectorCollectionHit> = doc_hits.into_values().collect();
        hits.sort_by(|a, b| b.score.partial_cmp(&a.score).unwrap_or(CmpOrdering::Equal));
        if hits.len() > query.limit {
            hits.truncate(query.limit);
        }

        Ok(VectorCollectionSearchResults { hits })
    }

    fn resolve_fields(&self, query: &VectorCollectionQuery) -> Result<Vec<String>> {
        ...
        if candidates.is_empty() {
            return Err(PlatypusError::invalid_config(
                "VectorCollection has no fields configured",
            ));
        }
    ...
    fn apply_field_selectors(&self, selectors: &[FieldSelector]) -> Result<Vec<String>> {
        if selectors.is_empty() {
            return Err(PlatypusError::invalid_argument(
                "VectorCollectionQuery fields selector list is empty",
            ));
        }
    ...
    fn merge_field_hits(
        &self,
        doc_hits: &mut HashMap<u64, VectorCollectionHit>,
        hits: Vec<FieldHit>,
        field_weight: f32,
...
impl Default for VectorCollectionQuery {
    fn default() -> Self {
        Self {
...
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
pub struct VectorCollectionSearchResults {
    #[serde(default)]
    pub hits: Vec<VectorCollectionHit>,
}
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct VectorCollectionHit {
    pub doc_id: u64,
    pub score: f32,
    #[serde(default)]
...
mod tests {
    ...
    fn collection_with_field(
        hits: Vec<FieldHit>,
        weight: f32,
    ) -> (VectorCollection, Arc<RecordingFieldWriter>) {
        ...
        let mut collection = VectorCollection::new(config, storage, None).expect("collection");
    ...
    fn sample_query(limit: usize) -> VectorCollectionQuery {
    ...
    fn collection_reports_stats() {
        ...
        let storage =
            Arc::new(MemoryStorage::new(MemoryStorageConfig::default())) as Arc<dyn Storage>;
        let collection = VectorCollection::new(config, storage, None).expect("collection");
    ...
    fn materialize_delegate_reader_builds_persistent_index() {
        ...
        let collection = VectorCollection::new(config, storage.clone(), None).expect("collection");
    ...
    fn registry_snapshot_persists_across_instances() {
        ...
        {
            let collection =
                VectorCollection::new(config.clone(), storage.clone(), None).expect("collection");
    ...
    fn manifest_mismatch_returns_error() {
        ...
        let err = VectorCollection::new(config, storage.clone(), None).expect_err("should fail");
        assert!(
            err.to_string()
