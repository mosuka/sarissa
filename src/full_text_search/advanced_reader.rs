...
impl crate::full_text::reader::PostingIterator for AdvancedPostingIterator {
    fn doc_id(&self) -> u64 {
...
impl crate::full_text::reader::IndexReader for AdvancedIndexReader {
    ...
    fn term_info(
        ...
        field: &str,
        term: &str,
    ) -> Result<Option<crate::full_text::reader::ReaderTermInfo>> {
        self.check_closed()?;
        ...
        if let Some(cached_info) = self.cache_manager.get_term_info(&cache_key) {
            return Ok(Some(crate::full_text::reader::ReaderTermInfo {
        ...
        if found {
            let reader_info = crate::full_text::reader::ReaderTermInfo {
                field: field.to_string(),
                term: term.to_string(),
