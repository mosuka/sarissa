...
use crate::full_text_index::merge_policy::MergePolicy;
...
impl BackgroundScheduler {
    ...
    pub fn start(&self) -> Result<()> {
        if self.running.load(Ordering::Acquire) {
            return Err(SarissaError::index("Background scheduler already running"));
        }
    ...
    pub fn submit_task(&self, task: BackgroundTask) -> Result<()> {
        if !self.running.load(Ordering::Acquire) {
            return Err(SarissaError::index("Background scheduler not running"));
        }
        ...
        self.task_sender
            .send(task)
            .map_err(|_| SarissaError::index("Failed to submit task to queue"))?;
