...
impl ParallelSearchEngine {
    ...
    pub fn add_index(
        ...
        let handle = IndexHandle::new(id, Arc::from(reader))
            .with_weight(weight);
    ...
    pub fn search(
        ...
        let successful_results = results.iter()
        ...
        let merge_strategy = options.merge_strategy
            ...
            match rx.recv_timeout(remaining) {
                ...
                Err(_) => {
                    ...
                    while results.len() < num_tasks {
                        results.push(TaskResult::timeout(
                            String::new(),
                            String::new(),
                            timeout,
                        ));
...
mod tests {
    ...
    fn create_test_reader() -> Box<dyn IndexReader> {
        ...
        schema.add_field("text", Box::new(TextField::new())).unwrap();
    ...
    fn test_index_management() {
        ...
        // Add indices
        engine.add_index("index1".to_string(), create_test_reader(), 1.0).unwrap();
        engine.add_index("index2".to_string(), create_test_reader(), 2.0).unwrap();
    ...
    fn test_search_with_indices() {
        ...
        // Add test indices
        engine.add_index("index1".to_string(), create_test_reader(), 1.0).unwrap();
        engine.add_index("index2".to_string(), create_test_reader(), 1.0).unwrap();
