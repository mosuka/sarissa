...
impl IndexManager {
    ...
    pub fn add_index(&self, handle: IndexHandle) -> Result<()> {
        ...
        let mut indices = self.indices.write().map_err(|_| {
            SarissaError::internal("Failed to acquire write lock on indices")
        })?;
        ...
        // Update total weight
        let mut total = self.total_weight.write().map_err(|_| {
            SarissaError::internal("Failed to acquire write lock on total weight")
        })?;
        *total += weight;
    ...
    pub fn get_index(&self, id: &str) -> Result<IndexHandle> {
        let indices = self.indices.read().map_err(|_| {
            SarissaError::internal("Failed to acquire read lock on indices")
        })?;
    ...
    pub fn get_total_weight(&self) -> Result<f32> {
        let total = self.total_weight.read().map_err(|_| {
            SarissaError::internal("Failed to acquire read lock on total weight")
        })?;
...
mod tests {
    ...
    fn test_index_handle_creation() {
        ...
        let handle = IndexHandle::new("test_index".to_string(), reader)
            .with_weight(2.0);
