...
use crate::parallel_lexical_index::config::IndexingOptions;
use crate::parallel_lexical_index::partitioner::DocumentPartitioner;
use crate::parallel_lexical_index::writer_manager::IndexWriterHandle;
...
impl BatchProcessor {
    ...
    pub fn add_document(&mut self, partition_index: usize, document: Document) -> Result<bool> {
        ...
        Ok(
            batch.is_full(self.config.max_batch_memory, self.config.max_batch_size)
...
mod tests {
    ...
    fn create_test_document(id: &str) -> Document {
        ...
        doc.add_field(
            "content".to_string(),
            FieldValue::Text("test content".to_string()),
        );
        doc
    }
    ...
    fn test_batch_processor() {
        ...
        assert!(
            !processor
                .add_document(0, create_test_document("doc1"))
                .unwrap()
        );
        assert!(
            processor
                .add_document(0, create_test_document("doc2"))
                .unwrap()
        ); // Should be ready
    ...
    fn test_partition_documents_into_batches() {
        ...
        let partition_batches =
            partition_documents_into_batches(documents, &partitioner, &config).unwrap();

        // Should have batches distributed across partitions
        assert!(!partition_batches.is_empty());

        let total_docs: usize = partition_batches
            .values()
