...
fn main() -> Result<()> {
    ...
    let tokens = create_test_tokens(&["the", "cat", "le", "chat", "der", "Hund", "el", "gato"]);
    demonstrate_filter(&multi_filter, tokens, "Articles in different languages")?;

    // Example 8: Case sensitivity
    println!("\n8. Case sensitivity:");
    let case_filter = StopFilter::from_words(vec!["the".to_string(), "AND".to_string()]);
    let tokens = create_test_tokens(&["the", "The", "THE", "and", "AND", "brown", "fox"]);
    ...
    for word in sample_words {
        println!(
            "  '{}': {}",
            word,
            if default_filter.is_stop_word(word) {
                "✓ stop word"
            } else {
                "✗ not stop word"
            }
