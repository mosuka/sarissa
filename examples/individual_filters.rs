...
fn main() -> Result<()> {
    ...
    let tokens = create_test_tokens(&[
        "Hello", "WORLD", "CamelCase", "UPPERCASE", "lowercase"
    ]);
    ...
    let boosted_tokens: Vec<Token> = token_vec.into_iter()
        .enumerate()
        .map(|(i, mut token)| {
    ...
    demonstrate_filter_with_boost(&boost_filter, final_tokens, "Text with different boost values")?;
    ...
    let custom_stop_filter = StopFilter::from_words(vec!["quick".to_string(), "lazy".to_string(), "over".to_string()]);
    let tokens = create_test_tokens(&[
    ...
    let preserve_stop_filter = StopFilter::from_words(vec!["the".to_string(), "and".to_string()])
...
fn demonstrate_filter(
    ...
    println!("Count: {} → {}", 
        input_count, filtered_tokens.len());
