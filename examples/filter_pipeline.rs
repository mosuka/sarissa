...
use sarissa::analysis::token_filter::{
    BoostFilter, Filter, LimitFilter, LowercaseFilter, RemoveEmptyFilter, StemFilter, StopFilter,
    StripFilter,
};
...
fn compare_pipelines(text: &str) -> Result<()> {
    ...
    println!(
        "Result: {:?}",
        result_a.iter().map(|t| &t.text).collect::<Vec<_>>()
    );
    ...
    println!(
        "Result: {:?}",
        result_b.iter().map(|t| &t.text).collect::<Vec<_>>()
    );
    ...
    println!(
        "Result: {:?}",
        result_c.iter().map(|t| &t.text).collect::<Vec<_>>()
    );
...
fn minimal_pipeline(text: &str) -> Result<()> {
    ...
    let final_tokens: Vec<_> = tokens.collect();
    println!(
        "Final result (tokenization only): {:?}",
        final_tokens.iter().map(|t| &t.text).collect::<Vec<_>>()
    );
...
fn maximum_pipeline(text: &str) -> Result<()> {
    ...
    println!(
        "  1. Strip: {:?}",
        step1.iter().map(|t| &t.text).collect::<Vec<_>>()
    );
    ...
    println!(
        "  2. Lowercase: {:?}",
        step2.iter().map(|t| &t.text).collect::<Vec<_>>()
    );
    ...
    println!(
        "  3. Remove empty: {:?}",
        step3.iter().map(|t| &t.text).collect::<Vec<_>>()
    );
    ...
    println!(
        "  4. Stop words: {:?}",
        step4.iter().map(|t| &t.text).collect::<Vec<_>>()
    );
    ...
    println!(
        "  5. Stemming: {:?}",
        step5.iter().map(|t| &t.text).collect::<Vec<_>>()
    );
