...
fn main() -> Result<()> {
    ...
    let tokens = create_test_tokens(&["\u{00A0}unicode\u{00A0}", "\u{2000}em\u{2000}", "normal"]);
...
fn demonstrate_filter(filter: &dyn Filter, tokens: TokenStream, description: &str) -> Result<()> {
    ...
    for (i, (input, output)) in input_clone.iter().zip(filtered_tokens.iter()).enumerate() {
        if input.text != output.text || output.is_stopped() {
            let status = if output.is_stopped() {
                " (→ stopped)"
            } else {
                ""
            };
