...
async fn main() -> Result<()> {
    ...
    doc1.add_field("content", FieldValue::Text("Rust is a systems programming language".to_string()));
    mock_readers[0].add_document(1, doc1);
    
    let mut doc2 = Document::new();
    doc2.add_field("title", FieldValue::Text("Rust Tutorial".to_string()));
    doc2.add_field("content", FieldValue::Text("Learn Rust programming step by step".to_string()));
    ...
    doc3.add_field("content", FieldValue::Text("Python is great for machine learning and programming".to_string()));
    ...
    doc4.add_field("content", FieldValue::Text("Data science with Python programming".to_string()));
    ...
    doc5.add_field("content", FieldValue::Text("JavaScript powers the modern web programming".to_string()));
    mock_readers[2].add_document(3, doc5);
    
    let mut doc6 = Document::new();
    doc6.add_field("title", FieldValue::Text("JavaScript Frameworks".to_string()));
    doc6.add_field("content", FieldValue::Text("Modern JavaScript frameworks for web development".to_string()));
    ...
    doc_fields.insert("content".to_string(), "Rust is a systems programming language".to_string());
    engine.add_document(1, doc_fields).await?;
    
    let mut doc_fields = HashMap::new();
    doc_fields.insert("title".to_string(), "Rust Tutorial".to_string());
    doc_fields.insert("content".to_string(), "Learn Rust programming step by step".to_string());
    ...
    doc_fields.insert("content".to_string(), "Python is great for machine learning and programming".to_string());
    engine.add_document(2, doc_fields).await?;
    
    let mut doc_fields = HashMap::new();
    doc_fields.insert("title".to_string(), "Python Data Science".to_string());
    doc_fields.insert("content".to_string(), "Data science with Python programming".to_string());
    ...
    doc_fields.insert("content".to_string(), "JavaScript powers the modern web programming".to_string());
    engine.add_document(3, doc_fields).await?;
    
    let mut doc_fields = HashMap::new();
    doc_fields.insert("title".to_string(), "JavaScript Frameworks".to_string());
    doc_fields.insert("content".to_string(), "Modern JavaScript frameworks for web development".to_string());
    ...
    for (i, result) in results.results.iter().enumerate() {
        ...
        if let Some(explanation) = &result.explanation {
            ...
            println!("    Keyword Contribution: {:.4}", explanation.keyword_contribution);
            println!("    Vector Contribution: {:.4}", explanation.vector_contribution);
        }
    }
    
    // Display timing breakdown
    println!("\n=== Search Time Breakdown ===");
    println!("Keyword Search: {:.2}ms", results.time_breakdown.keyword_search_ms);
    println!("Vector Search: {:.2}ms", results.time_breakdown.vector_search_ms);
    println!("Result Merging: {:.2}ms", results.time_breakdown.merge_ms);
    println!("Query Expansion: {:.2}ms", results.time_breakdown.expansion_ms);
    println!("Result Ranking: {:.2}ms", results.time_breakdown.ranking_ms);
    
    // Display cache statistics
    println!("\n=== Cache Statistics ===");
    println!("Cache Hit Rate: {:.2}%", results.cache_hit_rate() * 100.0);
    println!("Keyword Cache - Hits: {}, Misses: {}", 
        results.cache_stats.keyword_hits, 
        results.cache_stats.keyword_misses
    );
    println!("Vector Cache - Hits: {}, Misses: {}", 
        results.cache_stats.vector_hits, 
        results.cache_stats.vector_misses
    );
    
    // Batch search example
    println!("\n=== Batch Search Example ===");
    let queries = vec![
        ("rust programming", Box::new(TermQuery::new("content", "rust")) as Box<dyn sarissa::query::Query>),
        ("python machine learning", Box::new(TermQuery::new("content", "python")) as Box<dyn sarissa::query::Query>),
        ("javascript web", Box::new(TermQuery::new("content", "javascript")) as Box<dyn sarissa::query::Query>),
    ];
    ...
    println!("Batch search completed in {:.2}ms", elapsed.as_secs_f64() * 1000.0);
    ...
    for (i, results) in batch_results.iter().enumerate() {
        ...
        if let Some(top_result) = results.top_result() {
            println!("  Top result: Doc ID {} with score {:.4}", 
                top_result.doc_id, 
                top_result.combined_score
            );
        }
    }
    ...
    for strategy in strategies {
        ...
        doc2.add_field("content", FieldValue::Text("Python programming tutorial".to_string()));
        mock_reader.add_document(2, doc2);
        
        test_engine.add_index(
        ...
        doc_fields.insert("content".to_string(), "Python programming tutorial".to_string());
    ...
    println!("Keyword hits: {}, Vector hits: {}", 
        final_cache_stats.keyword_hits, 
        final_cache_stats.vector_hits
    );
